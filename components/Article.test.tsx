...
describe('Article sentence playback', () => {
  ...
  it('marks only the controller active sentence', () => {
    ...
    act(() => {
      tts.emit({
        status: 'playing',
        activeSegmentId: 'article-body-0-1-0',
        source: 'voxtral',
      });
    });
  ...
  it('delegates pause, resume, and stop to the controller facade', () => {
    ...
    act(() => tts.emit({ status: 'playing', activeSegmentId: 'article-title-0-0', source: 'voxtral' }));
    fireEvent.click(screen.getByTitle('Pause'));
    expect(tts.pauseSpeech).toHaveBeenCalledTimes(1);

    act(() => tts.emit({ status: 'paused', activeSegmentId: 'article-title-0-0', source: 'voxtral' }));
